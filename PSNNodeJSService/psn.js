		...
		request.post(psnURL.oauth, {form:{ // Refreshing the token with refresh_token to avoid login again
				...
				if (!error) {
					if ('access_token' in responseJSON && !('error' in responseJSON)) {
						...
						message = 'access_token obtained by using refresh_token: ' + accessToken;
						debug(message);
		...
		request.post(psnURL.oauth, {form:{ // Firts time login
				...
				if (!error) {
					if ('access_token' in responseJSON && !('error' in responseJSON)) {
						...
						message = 'SUCCESS: access_token/refresh_token obtained: ' + body;
						debug(message);
						if (typeof callback === "function") callback(message);
					}
					else {
					    message = 'ERROR: ' + JSON.stringify(responseJSON);
					    debug(message);
					    if (typeof callback === "function") callback(message);
					}
				}
